...
pub fn warp_system(mut query: Query<(&mut Transform, &Movement), With<Warp>>, q_windows: Query<&Window>, q_camera: Query<(&GlobalTransform, &Camera)>) {
    let window = q_windows.single();
    let (camera_transform, camera) = q_camera.single();

    for (mut transform, movement) in query.iter_mut() {
        if let Some(mut entity_pos) = camera.world_to_viewport(camera_transform, transform.translation) {
            ...
            transform.translation = camera.viewport_to_world_2d(camera_transform, entity_pos).unwrap().extend(0.0);
        }
    }
}
